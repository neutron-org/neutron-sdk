...
pub fn mint_native_receipt(deps: DepsMut<NeutronQuery>, env: Env, token_id: String, addr: Addr) -> NeutronResult<Response<NeutronMsg>>{
    ...
    MINTED_TOKENS.update(deps.storage, token_id.clone(), |s|{
        match s{
            None => Ok(token_count),
            Some(_) => Err(StdError::generic_err(format!("Token {}, was already migrated", token_id)))
        }
    })?;
    ...
    Ok(Response::new()
        ...
        .add_message(NeutronMsg::MintTokens { denom: format_token_denom(env, token_id, token_count), amount: MINT_AMOUNT.into(), mint_to_address: addr.to_string() }))
}
...
mod tests{
    ...
    fn right_address_generation() -> NeutronResult<()>{
        ...
        assert_eq!(neutron_addr, "neutron1phaxpevm5wecex2jyaqty2a4v02qj7qmnlcycg");
