...
RES=$(${NEUTROND_BIN} tx wasm store ${CONTRACT} --from ${USERNAME_1} --gas 50000000  --chain-id ${CHAIN_ID_1} --broadcast-mode=block --gas-prices 0.0025stake  -y --output json  --keyring-backend test --home ${HOME_1} --node tcp://127.0.0.1:16657)
...
RES=$(${NEUTROND_BIN} tx wasm instantiate $QUERIES_CONTRACT_CODE_ID "$INIT_QUERIES_CONTRACT" --from ${USERNAME_1} --admin ${ADMIN} -y --chain-id ${CHAIN_ID_1} --output json --broadcast-mode=block --label "init"  --keyring-backend test --gas-prices 0.0025stake --home ${HOME_1} --node tcp://127.0.0.1:16657)
echo $RES
...
RES=$(${NEUTROND_BIN} tx gov submit-proposal clear-contract-admin ${QUERIES_CONTRACT_ADDRESS} --title="clear contract admin" \
  --description="ssss" \
  --type="Text" \
...
RES=$(${NEUTROND_BIN} q gov proposals --chain-id ${CHAIN_ID_1} --home ${HOME_1} --node tcp://127.0.0.1:16657)
echo "--- q gov proposals:"
echo $RES
echo

# vote yes (w dominance of votes)
RES=$(${NEUTROND_BIN} tx gov vote 1 yes --from val1 --fees 5000stake --chain-id ${CHAIN_ID_1} -y --broadcast-mode=block --home ${HOME_1}  --keyring-backend test --node tcp://127.0.0.1:16657)
echo "--- tx gov vote:"
echo $RES
echo
...
# Is proposal passed?
RES=$(${NEUTROND_BIN} query gov proposal 1  --chain-id ${CHAIN_ID_1} --home ${HOME_1} --node tcp://127.0.0.1:16657)
echo "--- query gov proposal 1:"
echo $RES
echo

# Print new contract info (should have admin empty)
RES=$(${NEUTROND_BIN} query wasm contract ${QUERIES_CONTRACT_ADDRESS} --chain-id ${CHAIN_ID_1} --home ${HOME_1} --node tcp://127.0.0.1:16657)
