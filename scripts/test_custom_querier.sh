...
RES=$(${BIN} tx wasm execute $ICQ_CONTRACT_ADDRESS '{"register_delegator_delegations_query":{"connection_id":"connection-0","delegator":"cosmos10h9stc5v6ntgeygf5xf945njqq5h32r53uquvw", "validators": ["cosmosvaloper1qnk2n4nlkpw9xfqntladh74w6ujtulwnmxnh3k"], "update_period":5}}' --from ${TEST_WALLET}  -y --chain-id ${CHAINID} --output json --broadcast-mode=block --gas-prices 0.0025stake --gas 1000000 --keyring-backend test --home ${HOME} --node tcp://127.0.0.1:16657)
echo $RES | jq

echo "register_bank_total_supply_query"
RES=$(${BIN} tx wasm execute $ICQ_CONTRACT_ADDRESS '{"register_bank_total_supply_query":{"connection_id":"connection-0","denoms":["stake","stake"], "update_period":5}}' --from ${TEST_WALLET}  -y --chain-id ${CHAINID} --output json --broadcast-mode=block --gas-prices 0.0025stake --gas 1000000 --keyring-backend test --home ${HOME} --node tcp://127.0.0.1:16657)
echo $RES | jq
...
RES=$(${BIN} tx wasm execute $ICQ_CONTRACT_ADDRESS '{"register_distribution_fee_pool_query":{"connection_id":"connection-0", "update_period":5}}' --from ${TEST_WALLET}  -y --chain-id ${CHAINID} --output json --broadcast-mode=block --gas-prices 0.0025stake --gas 1000000 --keyring-backend test --home ${HOME} --node tcp://127.0.0.1:16657)
echo $RES | jq
...
RES=$(${BIN} tx wasm execute $ICQ_CONTRACT_ADDRESS '{"register_government_proposals_query":{"connection_id":"connection-0", "proposals_ids": [1, 2],"update_period":5}}' --from ${KEY}  -y --chain-id ${CHAINID} --output json --broadcast-mode=block --gas-prices 0.0025stake --gas 1000000 --keyring-backend test --home ${HOME} --node tcp://127.0.0.1:16657)
echo $RES | jq
...
echo "${BIN} q wasm contract-state smart $ICQ_CONTRACT_ADDRESS '{\"government_proposals\":{\"query_id\": 6}}' --chain-id ${CHAINID} --output json  --home ${HOME} --node tcp://127.0.0.1:16657"
