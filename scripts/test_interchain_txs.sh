...
url="http://127.0.0.1:1316/wasm/contract/$contract_address/smart/$query_b64_urlenc?encoding=base64"
ica_address=$(curl -s "$url" | jq -r '.result.smart' | base64 -d | jq -r '.[0]')
echo "ICA address: $ica_address"

tx_result=$("$GAIA_BIN" tx bank send "$ADDRESS_2" "$ica_address" 50000stake  \
    --chain-id "$CHAIN_ID_2" --broadcast-mode=block --gas-prices 0.0025stake \
...
curl -s "http://127.0.0.1:1317/staking/delegators/$ica_address/delegations" | jq '.result'
